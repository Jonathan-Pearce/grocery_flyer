"""
For every Loblaws store in data/loblaws/stores.json:
  1. Fetch the list of active publications from:
       GET /flyerkit/publications/loblaws
         ?languages[]=en&locale=en&access_token=...&store_code=<code>
  2. Merge new publications into data/loblaws/store_flyers.json
     (keyed by store_code; deduped by publication ID within each store list)
  3. For each newly-seen publication ID (no existing file in data/loblaws/flyers/):
       GET /flyerkit/publication/<id>/products
         ?display_type=all&locale=en&access_token=...
     Save products  →  data/loblaws/flyers/<id>.json
...
Logging (files are dated to the run date):
  logs/loblaws/<date>_verbose.log  — full output, mirrors the terminal
  logs/loblaws/<date>_summary.log  — one-page run summary
"""
...
PUBS_URL = "https://dam.flippenterprise.net/flyerkit/publications/loblaws"
PRODUCTS_URL = "https://dam.flippenterprise.net/flyerkit/publication/{pub_id}/products"

STORES_PATH = "data/loblaws/stores.json"
STORE_FLYERS_PATH = "data/loblaws/store_flyers.json"
FLYERS_DIR = "data/loblaws/flyers"
LOG_DIR = "logs/loblaws"
...
def main():
    ...
    log_summary(f"=== Loblaws Flyer Fetch — {today} ===")
    ...
    if not all_pubs:
        ...
        for pub_id, pub_meta in all_pubs.items():
            ...
            # File may already exist if this pub_id was seen via another store
            # in a previous run but wasn't in that store's store_flyers entry yet.
