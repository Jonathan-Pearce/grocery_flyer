...
class TestFetchStorePublications:
    def test_returns_list_response_directly(self):
        ...
        with patch("fetchers.flipp.get", return_value=pubs):
            result = fetch_store_publications(_brand(), "100")
        assert result == pubs

    def test_extracts_flyers_key_from_dict(self):
        pubs = [{"id": 1}]
        with patch("fetchers.flipp.get", return_value={"flyers": pubs}):
            result = fetch_store_publications(_brand(), "100")
        assert result == pubs

    def test_extracts_publications_key_from_dict(self):
        pubs = [{"id": 2}]
        with patch("fetchers.flipp.get", return_value={"publications": pubs}):
            result = fetch_store_publications(_brand(), "100")
        assert result == pubs

    def test_returns_empty_list_on_none_response(self):
        with patch("fetchers.flipp.get", return_value=None):
            result = fetch_store_publications(_brand(), "100")
        assert result == []

    def test_returns_empty_list_when_dict_has_no_known_key(self):
        with patch("fetchers.flipp.get", return_value={"other": []}):
            result = fetch_store_publications(_brand(), "100")
        assert result == []

    def test_returns_empty_list_when_flyers_value_is_not_a_list(self):
        with patch("fetchers.flipp.get", return_value={"flyers": "bad_value"}):
            result = fetch_store_publications(_brand(), "100")
        assert result == []
...
class TestFetchPublicationProducts:
    def test_returns_list_of_products(self):
        ...
        with patch("fetchers.flipp.get", return_value=products):
...
class TestFetchStore:
    def test_returns_store_data_on_success(self):
        ...
        with patch("fetchers.flipp.requests.get", return_value=_mock_resp(200, store_data)):
            result = fetch_store(_brand(), 1001)
        assert result == store_data

    def test_returns_none_on_non_200(self):
        with patch("fetchers.flipp.requests.get", return_value=_mock_resp(404)):
            result = fetch_store(_brand(), 9999)
        assert result is None

    def test_returns_none_when_response_is_empty(self):
        with patch("fetchers.flipp.requests.get", return_value=_mock_resp(200, {})):
            result = fetch_store(_brand(), 1001)
        assert result is None

    def test_returns_none_on_request_exception(self):
        import requests as req_lib
        with patch("fetchers.flipp.requests.get", side_effect=req_lib.RequestException("timeout")):
            result = fetch_store(_brand(), 1001)
        assert result is None

    def test_returns_none_on_json_decode_error(self):
        with patch("fetchers.flipp.requests.get", return_value=_mock_resp(200, json_exc=ValueError("bad json"))):
            result = fetch_store(_brand(), 1001)
        assert result is None
