"""
Scan Flipp store codes for one or all Loblaws portfolio brands and merge
results into data/<folder>/stores.json.

Usage:
  python fetch_loblaws_portfolio_stores.py                 # all brands with confirmed slugs
  python fetch_loblaws_portfolio_stores.py --brand loblaws
  python fetch_loblaws_portfolio_stores.py --brand nofrills --start 10001 --end 15000
"""
...
from flipp import DELAY, LOBLAWS_PORTFOLIO, Brand, fetch_store
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Scan Loblaws portfolio store codes")
    parser.add_argument(
        "--brand",
        metavar="FOLDER",
        help="folder name of a single brand to scan (e.g. loblaws, nofrills). "
    ...
    for brand in brands:
        ...
        if brand.slug is None:
            print(f"  [!] Slug not yet confirmed for {brand.name} — skipping.\n"
                  f"      Add the Flipp URL slug to LOBLAWS_PORTFOLIO in flipp.py to enable.\n")
            continue
