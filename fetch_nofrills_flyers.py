"""
For every No Frills store in data/nofrills/stores.json:
  1. Fetch the list of active publications from:
       GET /flyerkit/publications/nofrills
         ?languages[]=en&locale=en&access_token=...&store_code=<code>
  2. Merge new publications into data/nofrills/store_flyers.json
     (keyed by store_code; deduped by publication ID within each store list)
  3. For each newly-seen publication ID (no existing file in data/nofrills/flyers/):
       GET /flyerkit/publication/<id>/products
         ?display_type=all&locale=en&access_token=...
     Save products  →  data/nofrills/flyers/<id>.json
...
Logging (files are dated to the run date):
  logs/nofrills/<date>_verbose.log  — full output, mirrors the terminal
  logs/nofrills/<date>_summary.log  — one-page run summary
"""
...
PUBS_URL = "https://dam.flippenterprise.net/flyerkit/publications/nofrills"
PRODUCTS_URL = "https://dam.flippenterprise.net/flyerkit/publication/{pub_id}/products"

STORES_PATH = "data/nofrills/stores.json"
STORE_FLYERS_PATH = "data/nofrills/store_flyers.json"
FLYERS_DIR = "data/nofrills/flyers"
LOG_DIR = "logs/nofrills"
...
def main():
    ...
    log_summary(f"=== No Frills Flyer Fetch — {today} ===")
